...
if (modoR3 != AUTO) return;

  bool ventanaActiva = false;
  int diaActual = day;             // 0–6
  int diaAyer   = (day + 6) % 7;   // día anterior
  ...
  if (startR3 < offR3) {
    ...
    // Tramo 1: desde startR3 hasta 23:59 del día habilitado
    if (diasRiego[diaActual] == 1 &&
        currentTime >= startR3) {
      ventanaActiva = true;
    }

    // Tramo 2: desde 00:00 hasta offR3 del día siguiente
    if (diasRiego[diaAyer] == 1 &&   // el riego arrancó AYER
        currentTime < offR3) {
      ventanaActiva = true;
    }
  }
  ...
  if (ventanaActiva) {
    ...
    digitalWrite(RELAY3, HIGH); // OFF si tu relay es activo en LOW
    R3estado = HIGH;
    enRiego  = false;
    // opcional: previousMillisRiego = millis();  // resetea la pausa
  }
...
void handleControlR1() {
  ...
  html += "    <form action='/controlR1Timer' method='POST'><input type='submit' value='TIMER'></form>";
  html += "    <a href='/control'><button>VOLVER</button></a>";
  html += "  </div>";
  html += "</div>";
...
void handleControlR1Timer() {
    modoR1 = 6; // Modo TIMER
    Guardado_General();
    handleConfirmation(getRelayName(R1name) + " en modo TIMER", "/controlR1"); 
}
...
void handleControlR5() {
  ...
  html += "    <form action='/controlR5Timer' method='POST'><input type='submit' value='TIMER'></form>";
  html += "    <a href='/control'><button>VOLVER</button></a>";
  html += "  </div>";
  html += "</div>";
...
void handleControlR5Timer() {
    modoR5 = 6; // Modo TIMER
    Guardado_General();
    handleConfirmation(getRelayName(R5name) + " en modo TIMER", "/controlR5");
}
